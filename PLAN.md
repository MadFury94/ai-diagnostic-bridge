...
- [ ] **T04.1** Implement `class-diagnostic-manager.php` with a registry mapping allowlisted check IDs to diagnostic classes.
- [ ] **T04.2** Implement `class-rest-api.php` and register `/ai-diagnostic/v1` routes with authentication permission callbacks.
- [ ] **T04.3** Add `GET /diagnostic` and `POST /diagnostic`; validate `checks` against the registry and reject unknown values.
- [ ] **T04.4** Add route handlers for `/site`, `/health`, `/errors`, `/plugins`, `/themes`, `/rest-api`, `/performance`, `/woocommerce`, and the SEO/image/link routes.
- [ ] **T04.5** Ensure request data cannot select functions, files, SQL, shell commands, WP-CLI commands, hooks, or arbitrary classes.
- [ ] **T04.6** Add REST permission, invalid-request, unknown-check, and combined-response tests.
- [ ] **T04.7** Perform an unauthenticated and authenticated REST smoke test against WordPress.
...
- [ ] **T05.1** Implement `class-site-health.php` for safe WordPress/PHP/database versions, URLs, HTTPS, multisite, permalinks, timezone, locale, limits, active theme/plugins, debug state, cron, and REST availability.
- [ ] **T05.2** Implement deterministic health findings and statuses without exposing configuration contents or private data.
...
- [ ] **T06.1** Implement `class-plugins.php` using WordPress plugin APIs for installed, active/network-active, version, author, slug, updates, and deterministic observations.
- [ ] **T06.2** Implement `class-themes.php` for active, parent/child, version, stylesheet metadata, directory, and update observations with path redaction.
...
- **Current task:** `T01.5`
- **Last completed task:** `T03.5` (foundation implementation)
- **Files changed in last session:** bootstrap, plugin lifecycle, response, authentication, activity-log, README/readme, `.gitignore`, uninstall files
- **Tests/checks run:** attempted `php -l`; PHP executable is unavailable in the current shell
- **Known blockers:** no PHP runtime or disposable WordPress test site is available here
- **Next action:** install/provide PHP 8.1+ and a WordPress 6.5+ test site, run syntax/activation checks, then implement T02.5/T03.3/T03.4/T03.6 and Phase 4 REST controller
- **Do not redo:** T00.1, T00.3, T00.4, T01.1–T01.4, T02.1–T02.3, T03.1, T03.2, T03.4, and T03.5 are implemented. T02.4 remains partial until REST pagination helpers are added.
